...
bool CudfHiveConfig::scanDiagnosticsEnabled() const {
  return config_->get<bool>(kScanDiagnosticsEnabled, false);
}

bool CudfHiveConfig::scanDiagnosticsEnabledSession(
    const config::ConfigBase* session) const {
  return session->get<bool>(
      kScanDiagnosticsEnabledSession,
      config_->get<bool>(kScanDiagnosticsEnabled, false));
}
